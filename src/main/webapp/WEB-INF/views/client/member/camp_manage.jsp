...
<!-- table -->
	...
	<table class="table">
		...
		<!-- for(List<BookingInfo> list : resultlist) -->
		<c:forEach items="${list}" var="list" varStatus="index">
			<tr>
				<th scope="row" style="padding-top: 25px;">${list.rnum}</th>
		...
		<thead>
		<tr style="text-align: center">
		<th scope="col">총사이트수</th>
		<th scope="col">현재 사용 중</th>
		<th scope="col">체크인대기</th>
		<th scope="col">체크인아웃 완료</th>
		</tr>
		</thead>
		<tbody>
		<tr style="text-align:center">
		...
		<nav aria-label="...">
		  <ul class="pagination justify-content-center">
		 	<c:choose>
		 		...
		 		<c:otherwise>
					<li class="page-item">
				      <a class="page-link" onclick="start_camp_manage(true, true, true, '${page.startPage-1}')">Previous</a>
				    </li>
		 		</c:otherwise>
		 	</c:choose>
		 	...
		 	<c:choose>
		 		...
		 		<c:otherwise>
		 		    <li class="page-item">
				      <a class="page-link" onclick="start_camp_manage(true, true, true, '${page.endPage+1}')">Next</a>
				    </li>
		 		</c:otherwise>
		 	</c:choose>
		  </ul>
